...
def _can_access_broadcast(role: str, user_id: int, bc: dict) -> bool:
    """Root видит все рассылки, owner — только свои."""
    if role == "root":
        return True
    return bc["created_by"] == user_id
...
async def cb_view(callback: CallbackQuery):
    ...
    # ИСПРАВЛЕНИЕ: owner видит только свои рассылки
    if not _can_access_broadcast(role, callback.from_user.id, bc):
        return await callback.answer("⛔ Это не ваша рассылка.", show_alert=True)
...
async def cb_delete_confirm(callback: CallbackQuery):
    ...
    # ИСПРАВЛЕНИЕ: owner может удалять только свои рассылки
    if not _can_access_broadcast(role, callback.from_user.id, bc):
        return await callback.answer("⛔ Это не ваша рассылка.", show_alert=True)
...
async def cb_confirm_delete(callback: CallbackQuery, scheduler: AsyncIOScheduler):
    ...
    # ИСПРАВЛЕНИЕ: финальная проверка перед удалением
    if not _can_access_broadcast(role, callback.from_user.id, bc):
        return await callback.answer("⛔ Это не ваша рассылка.", show_alert=True)
...
async def cb_edit(callback: CallbackQuery, state: FSMContext):
    ...
    # ИСПРАВЛЕНИЕ: owner может редактировать только свои рассылки
    if not _can_access_broadcast(role, callback.from_user.id, bc):
        return await callback.answer("⛔ Это не ваша рассылка.", show_alert=True)
